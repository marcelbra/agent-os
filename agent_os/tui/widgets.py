...
class StructuredEditor(Widget):
    ...
    def compose(self):
        for attr_key, label, _kinds, readonly in FIELD_DEFS:
            with Horizontal(classes="se-row", id=f"se-row-{attr_key}"):
                ...
                yield Input(id=f"se-inp-{attr_key}", disabled=readonly)
        yield Rule(classes="se-sep")
        yield DetailTextArea("", id="se-body", language="markdown", theme="vscode_dark")
    ...
    def load(self, item: Any, kind: str) -> None:
        ...
        ta = self.query_one("#se-body", DetailTextArea)
        ta.load_text(body)
        ta.move_cursor((0, 0))
    ...
    def focus_first(self) -> None:
        ...
        for attr_key, _label, kinds, readonly in FIELD_DEFS:
            if self._kind in kinds and not readonly:
                self.query_one(f"#se-inp-{attr_key}", Input).focus()
                return
        self.query_one("#se-body", DetailTextArea).focus()
    ...
    def editor_text(self) -> str:
        ...
        body = self.query_one("#se-body", DetailTextArea).text
